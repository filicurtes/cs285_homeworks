...
def lander_exploration_schedule_1(num_timesteps):
...
class ExponentialSchedule(object):
    def __init__(self,num_iters, eps_0, decay_start_rate,outside_value=None):

        self.epsDecayEpisode = num_iters/decay_start_rate
        self.eps_0 =eps_0
        self.outside_value=outside_value
    def value(self, t):
        self.outside_value = self.eps_0 * np.power(0.1,(t/self.epsDecayEpisode))
...
if __name__ == '__main__':
    ...
    lander_exploration_schedule_1=lander_exploration_schedule_1(500000)
    eps_list = []
    for el in train_steps:
        eps=lander_exploration_schedule_1.value(el)
        #print(eps)
        eps_list.append(eps)
    ...
    ax.set_title(f"DQN Eps exponential decay Return")
    ax2 = ax.twinx()
    ...
    fig.tight_layout()
    #plt.show()
    plt.savefig(f"Eps 6 exponential decay DQN Rewards")
